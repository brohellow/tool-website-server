...
const initDatabase = async () => {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      // 添加联合索引，加速浏览记录查询
      db.run(`CREATE INDEX IF NOT EXISTS idx_tool_views_user_tool ON tool_views(user_id, tool_id)`);
      ...
      db.get('SELECT COUNT(*) AS count FROM tools', (err, result) => {
        ...
        if (result.count === 0) {
          ...
          toolsData.forEach(tool => {
            insertStmt.run(tool.id, tool.name, tool.category, tool.description, tool.icon, tool.featured, tool.usage_count, tool.views_count, tool.created_at);
          });
          ...
          insertStmt.finalize(() => {
            ...
            usersData.forEach(user => {
              insertUserStmt.run(user.id, user.email, user.password, user.username, user.avatar_url, user.bio, user.created_at);
            });
            ...
            insertUserStmt.finalize(() => {
              ...
              commentsData.forEach(comment => {
                insertCommentStmt.run(comment.id, comment.user_id, comment.tool_id, comment.parent_id, comment.content, comment.rating, comment.likes, comment.created_at);
              });
